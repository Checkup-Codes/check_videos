...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="h-[100vh] overflow-auto"><div class="sticky top-0 z-20 bg-sidebar"><div style="${ssrRenderStyle(showCategories.value ? null : { display: "none" })}" class="grid grid-cols-3 gap-1 px-4 pb-3 text-sm"><!--[-->`);
      ...
      ssrRenderList(writes.value, (write) => {
        ...
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="py-1 font-bold"${_scopeId}>${ssrInterpolate(write.title)}</div><div class="flex"${_scopeId}><div class="py-0.5 text-sm text-gray-400"${_scopeId}>${ssrInterpolate(formatDate(write.updated_at))}</div><div class="px-5 py-0.5 text-sm text-gray-400"${_scopeId}>${ssrInterpolate(write.views_count)} Görüntülenme</div></div>`);
            } else {
              return [
                createVNode("div", { class: "py-1 font-bold" }, toDisplayString(write.title), 1),
                createVNode("div", { class: "flex" }, [
                  createVNode("div", { class: "py-0.5 text-sm text-gray-400" }, toDisplayString(formatDate(write.updated_at)), 1),
