...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="mx-10 mb-2 mt-4 flex justify-between space-x-4 rounded bg-gray-100 px-2 py-1 text-gray-400"><div class="flex"><label for="versionDropdown" class="font-semibold text-gray-700">Versiyon Seç:</label><select id="versionDropdown" class="mx-5 h-7 rounded-md border border-gray-300 bg-white py-0 pl-2 text-gray-700 shadow-sm focus:border-gray-500 focus:outline-none focus:ring focus:ring-gray-200"><!--[-->`);
      ...
      if (unref(props).auth.user) {
        _push(`<button class="rounded bg-red-200 px-5 text-black hover:bg-red-300"> Sil </button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<button id="" class="rounded bg-green-200 px-5 text-black hover:bg-green-300"> Kaydet </button></div></div><div id="excalidraw-container" class="relative h-[650px] w-full pt-3"><div id="excali" class="h-full w-full"></div></div><!--]-->`);
    };
  }
};
