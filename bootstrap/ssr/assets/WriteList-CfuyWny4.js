import { inject, computed, ref, onActivated, onDeactivated, watch, nextTick, onMounted, onUnmounted, mergeProps, unref, withCtx, createVNode, toDisplayString, openBlock, createBlock, createCommentVNode, useSSRContext } from "vue";
...
const _sfc_main = /* @__PURE__ */ Object.assign({ name: "WriteList" }, {
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-1 p-2" }, _attrs))} data-v-7227c32e><!--[-->`);
      ssrRenderList(filteredWrites.value, (write) => {
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="mb-1 flex items-start gap-2" data-v-7227c32e${_scopeId}><h4 class="${ssrRenderClass([activeWrite.value === getActiveWritePath(write) ? "text-primary-foreground" : "text-foreground", "line-clamp-2 flex-1 text-[11px] font-medium leading-tight"])}" data-v-7227c32e${_scopeId}>${ssrInterpolate(write.title)}</h4><div class="flex shrink-0 items-center gap-1" data-v-7227c32e${_scopeId}>`);
              if (write.status === "private") {
                _push2(`<span class="flex h-4 w-4 items-center justify-center rounded bg-yellow-500/10 text-yellow-600 dark:text-yellow-400" title="Gizli yazı" data-v-7227c32e${_scopeId}>`);
              ...
              if (write.status === "link_only") {
                _push2(`<span class="flex h-4 w-4 items-center justify-center rounded bg-blue-500/10 text-blue-600 dark:text-blue-400" title="Sadece link" data-v-7227c32e${_scopeId}>`);
                _push2(ssrRenderComponent(_sfc_main$2, { class: "h-2.5 w-2.5" }, null, _parent2, _scopeId));
                _push2(`</span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div></div><div class="${ssrRenderClass([activeWrite.value === getActiveWritePath(write) ? "text-primary-foreground/70" : "text-muted-foreground", "flex flex-col gap-1 text-[10px] sm:flex-row sm:items-center sm:justify-between"])}" data-v-7227c32e${_scopeId}><div class="flex items-center gap-3" data-v-7227c32e${_scopeId}><span class="flex items-center gap-1" data-v-7227c32e${_scopeId}>`);
              _push2(ssrRenderComponent(_sfc_main$3, { class: "h-3 w-3" }, null, _parent2, _scopeId));
              _push2(`<span data-v-7227c32e${_scopeId}>${ssrInterpolate(formatDate(write.created_at))}</span></span><span class="flex items-center gap-1" data-v-7227c32e${_scopeId}>`);
              _push2(ssrRenderComponent(_sfc_main$4, { class: "h-3 w-3" }, null, _parent2, _scopeId));
              _push2(`<span data-v-7227c32e${_scopeId}>${ssrInterpolate(formatNumber(write.views_count))}</span></span></div></div>`);
            } else {
              return [
                createVNode("div", { class: "mb-1 flex items-start gap-2" }, [
                  createVNode("h4", {
                    class: ["line-clamp-2 flex-1 text-[11px] font-medium leading-tight", activeWrite.value === getActiveWritePath(write) ? "text-primary-foreground" : "text-foreground"]
                  ...
                  createVNode("div", { class: "flex shrink-0 items-center gap-1" }, [
                    write.status === "private" ? (openBlock(), createBlock("span", {
                      key: 0,
                      class: "flex h-4 w-4 items-center justify-center rounded bg-yellow-500/10 text-yellow-600 dark:text-yellow-400",
                      title: "Gizli yazı"
                    }, [
                    ...
                    write.status === "link_only" ? (openBlock(), createBlock("span", {
                      key: 1,
                      class: "flex h-4 w-4 items-center justify-center rounded bg-blue-500/10 text-blue-600 dark:text-blue-400",
                      title: "Sadece link"
                    }, [
                ...
                createVNode("div", {
                  class: ["flex flex-col gap-1 text-[10px] sm:flex-row sm:items-center sm:justify-between", activeWrite.value === getActiveWritePath(write) ? "text-primary-foreground/70" : "text-muted-foreground"]
                }, [
      ...
      if (filteredWrites.value.length === 0) {
        _push(`<div class="flex h-32 items-center justify-center text-center text-muted-foreground" data-v-7227c32e><div data-v-7227c32e>Henüz yazı bulunmuyor</div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const WriteList = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-7227c32e"]]);
export {
