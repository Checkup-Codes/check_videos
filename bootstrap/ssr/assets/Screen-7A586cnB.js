import { computed, ref, withCtx, unref, createTextVNode, createVNode, withModifiers, withDirectives, createBlock, createCommentVNode, vModelText, openBlock, toDisplayString, vModelSelect, vModelCheckbox, Fragment, renderList, withKeys, useSSRContext } from "vue";
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (errors.value.meaning) {
              _push2(`<label class="label"${_scopeId}><span class="label-text-alt text-error"${_scopeId}>${ssrInterpolate(errors.value.meaning)}</span></label>`);
            } else {
              _push2(`<!---->`);
            }
            ...
            if (props.languagePacks.length === 0) {
              ...
              ssrRenderList(props.languagePacks, (pack) => {
                _push2(`<div class="form-control"${_scopeId}><label class="label cursor-pointer"${_scopeId}><span class="label-text"${_scopeId}>${ssrInterpolate(pack.name)} <span class="badge badge-sm badge-outline ml-1"${_scopeId}>${ssrInterpolate(pack.language)}</span></span><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(unref(form).language_pack_ids) ? ssrLooseContain(unref(form).language_pack_ids, pack.id) : unref(form).language_pack_ids) ? " checked" : ""}${ssrRenderAttr("value", pack.id)} class="checkbox checkbox-sm"${_scopeId}></label></div>`);
              });
              _push2(`<!--]--></div>`);
            }
