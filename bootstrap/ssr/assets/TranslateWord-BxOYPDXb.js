...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const isCorrect = computed(() => {
      var _a, _b;
      return ((_b = (_a = currentWord.value) == null ? void 0 : _a.meaning) == null ? void 0 : _b.toLowerCase()) === userInput.value.toLowerCase();
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (quizFinished.value) {
        ...
        _push(`<input${ssrRenderAttr("value", userInput.value)}${ssrIncludeBooleanAttr(answered.value) ? " disabled" : ""} type="text" placeholder="Enter the translation" class="w-full rounded-lg border border-gray-300 p-3 text-base text-gray-900 shadow-sm placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100">`);
        if (answered.value) {
          _push(`<p class="${ssrRenderClass([isCorrect.value ? "text-green-600" : "text-red-600", "mt-4 text-sm font-medium"])}">${ssrInterpolate(feedbackMessage.value)}</p>`);
        } else {
          _push(`<!---->`);
        }
        if (!answered.value) {
        ...
        if (answered.value) {
          _push(`<button class="mt-4 w-full rounded-lg bg-black px-4 py-2 text-sm font-medium text-white shadow-md transition hover:bg-gray-700">${ssrInterpolate(isLastQuestion.value ? "Analyze" : "Next")}</button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
