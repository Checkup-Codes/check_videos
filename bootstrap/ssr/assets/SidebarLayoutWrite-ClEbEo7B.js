    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(writes.value, (write) => {
        ...
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="py-1 font-bold"${_scopeId}>${ssrInterpolate(write.title)}</div><div class="flex"${_scopeId}><div class="py-0.5 text-sm text-gray-400"${_scopeId}>${ssrInterpolate(formatDate(write.created_at))}</div><div class="px-5 py-0.5 text-sm text-gray-400"${_scopeId}>${ssrInterpolate(write.views_count)} Görüntülenme</div></div>`);
            } else {
              return [
