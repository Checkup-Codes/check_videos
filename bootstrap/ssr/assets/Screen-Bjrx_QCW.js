        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(customers.value, (customer) => {
              _push2(`<option${ssrRenderAttr("value", customer.id)} data-v-d13e4fce${ssrIncludeBooleanAttr(Array.isArray(unref(form).customer_id) ? ssrLooseContain(unref(form).customer_id, customer.id) : ssrLooseEqual(unref(form).customer_id, customer.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(customer.first_name)} ${ssrInterpolate(customer.last_name)}</option>`);
            });
