...
const _sfc_main = /* @__PURE__ */ Object.assign({ name: "CategoryTree" }, {
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-1" }, _attrs))} data-v-5ec7d621><!--[-->`);
      ssrRenderList(filteredCategories.value, (category) => {
        _push(`<div class="space-y-1" data-v-5ec7d621>`);
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="flex items-center justify-between" data-v-5ec7d621${_scopeId}><span class="truncate" data-v-5ec7d621${_scopeId}>${ssrInterpolate(category.name)}</span>`);
              if (category.tests_count) {
                _push2(`<span class="inline-flex items-center rounded-full border border-border bg-secondary px-2 py-0.5 text-xs font-semibold text-secondary-foreground" data-v-5ec7d621${_scopeId}>${ssrInterpolate(category.tests_count)}</span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div>`);
        ...
        if (category.children && category.children.length > 0) {
          _push(`<div class="ml-4 space-y-0.5" data-v-5ec7d621><!--[-->`);
          ssrRenderList(category.children, (child) => {
            _push(ssrRenderComponent(unref(Link), {
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  _push2(`<div class="flex items-center justify-between" data-v-5ec7d621${_scopeId}><span class="truncate" data-v-5ec7d621${_scopeId}>${ssrInterpolate(child.name)}</span>`);
                  if (child.tests_count) {
                    _push2(`<span class="inline-flex items-center rounded-full border border-border bg-secondary px-1.5 py-0.5 text-[10px] font-semibold text-secondary-foreground" data-v-5ec7d621${_scopeId}>${ssrInterpolate(child.tests_count)}</span>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  _push2(`</div>`);
      ...
      if (filteredCategories.value.length === 0) {
        _push(`<div class="flex h-32 items-center justify-center text-center text-muted-foreground opacity-50" data-v-5ec7d621><div data-v-5ec7d621>Henüz kategori bulunmuyor</div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const CategoryTree = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-5ec7d621"]]);
export {
