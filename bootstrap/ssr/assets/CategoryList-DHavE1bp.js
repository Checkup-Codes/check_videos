      ...
      ssrRenderList(__props.categories, (category) => {
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            var _a, _b;
            if (_push2) {
              _push2(`<h3 class="${ssrRenderClass([isActive(category) ? "text-primary-foreground" : "text-foreground", "text-sm font-medium leading-tight"])}"${_scopeId}>${ssrInterpolate(category.name)}</h3><div class="${ssrRenderClass([isActive(category) ? "text-primary-foreground/70" : "text-muted-foreground", "flex items-center gap-3 text-xs"])}"${_scopeId}><span class="flex items-center gap-1"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"${_scopeId}></path></svg><span class="truncate"${_scopeId}>${ssrInterpolate(((_a = category.bookmarks) == null ? void 0 : _a.length) || 0)} yer imi</span></span></div>`);
            } else {
              return [
                createVNode("h3", {
                  class: ["text-sm font-medium leading-tight", isActive(category) ? "text-primary-foreground" : "text-foreground"]
                }, toDisplayString(category.name), 3),
                createVNode("div", {
                  class: ["flex items-center gap-3 text-xs", isActive(category) ? "text-primary-foreground/70" : "text-muted-foreground"]
                }, [
