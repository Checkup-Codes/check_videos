import { ref, watch, onMounted, onUnmounted, mergeProps, unref, useSSRContext } from "vue";
  ...
  setup(__props) {
    ...
    const imageHandler = () => {
      ...
      `;
          ...
          showToastMessage("Error uploading image: " + (((_b = (_a = error.response) == null ? void 0 : _a.data) == null ? void 0 : _b.message) || error.message), "alert-error");
        } finally {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div><div><div class="form-control w-full"><label class="label"><span class="label-text">Yayınlama Tarihi</span></label><div class="relative"><input type="datetime-local"${ssrRenderAttr("value", unref(form).published_at)} class="${ssrRenderClass([{ "input-error": errors.value.published_at || unref(form).errors.published_at }, "input-bordered input w-full"])}"></div>`);
      ...
      _push(`</div></div><div><label class="label"><span class="label-text">Durumu</span></label><select class="${ssrRenderClass([{ "select-error": errors.value.status || unref(form).errors.status }, "select-bordered select w-full bg-base-100 text-neutral-content"])}"><option value="draft"${ssrIncludeBooleanAttr(Array.isArray(unref(form).status) ? ssrLooseContain(unref(form).status, "draft") : ssrLooseEqual(unref(form).status, "draft")) ? " selected" : ""}>Şablon</option><option value="published"${ssrIncludeBooleanAttr(Array.isArray(unref(form).status) ? ssrLooseContain(unref(form).status, "published") : ssrLooseEqual(unref(form).status, "published")) ? " selected" : ""}>Yayında</option><option value="private"${ssrIncludeBooleanAttr(Array.isArray(unref(form).status) ? ssrLooseContain(unref(form).status, "private") : ssrLooseEqual(unref(form).status, "private")) ? " selected" : ""}>Gizli</option></select>`);
