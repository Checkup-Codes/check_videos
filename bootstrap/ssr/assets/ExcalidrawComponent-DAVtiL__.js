...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="bg-theme-background px-2 lg:p-3"><div class="flex justify-between rounded-md px-1 py-2 text-theme-text sm:mx-3 md:mx-5"><div class="flex"><label for="versionDropdown" class="my-auto px-3 font-semibold">Versiyon Seç:</label><select id="versionDropdown" class="mx-5 border py-0 pl-2 text-theme-text shadow-sm focus:outline-none focus:ring focus:ring-gray-200"><!--[-->`);
      ...
      if (unref(props).auth.user) {
        _push(`<button class="rounded px-5 hover:underline">Sil</button>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(props).auth.user) {
        _push(`<button class="rounded px-5 hover:underline">Kaydet</button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div><div id="excalidraw-container" class="h-[calc(79vh)] w-full"><div id="excali" class="h-full w-full"></div></div></div><!--]-->`);
    };
  }
};
