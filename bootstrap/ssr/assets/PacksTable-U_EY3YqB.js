    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="p-6"${_scopeId}><div class="mb-6 flex items-center justify-between"${_scopeId}><div${_scopeId}><h2 class="text-xl font-bold text-base-content"${_scopeId}>Dil Paketleri</h2><p class="text-base-content/70 text-sm"${_scopeId}>Toplam Paket Sayısı: ${ssrInterpolate(__props.languagePacks.length)}</p></div>`);
            if (isLoggedIn.value) {
            ...
            _push2(`</div><div class="card bg-base-100 shadow-lg"${_scopeId}><div class="card-body"${_scopeId}><div class="flex flex-wrap gap-4"${_scopeId}><div class="min-w-[200px] flex-1"${_scopeId}><input type="text"${ssrRenderAttr("value", searchQuery.value)} placeholder="Kelime veya anlam ara..." class="input-bordered input w-full"${_scopeId}></div><div class="min-w-[200px] flex-1"${_scopeId}><select class="select-bordered select w-full"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "") : ssrLooseEqual(statusFilter.value, "")) ? " selected" : ""}${_scopeId}>Tüm Durumlar</option><option value="0"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "0") : ssrLooseEqual(statusFilter.value, "0")) ? " selected" : ""}${_scopeId}>Öğrenilmedi</option><option value="1"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "1") : ssrLooseEqual(statusFilter.value, "1")) ? " selected" : ""}${_scopeId}>Öğreniliyor</option><option value="2"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "2") : ssrLooseEqual(statusFilter.value, "2")) ? " selected" : ""}${_scopeId}>Öğrenildi</option></select></div><button class="btn btn-outline btn-sm"${_scopeId}>Filtreleri Temizle</button></div></div>`);
            if (isLoading.value) {
              _push2(`<div class="p-8 text-center"${_scopeId}><div class="loading loading-spinner loading-lg"${_scopeId}></div><p class="text-base-content/70 mt-2"${_scopeId}>Kelime listesi yükleniyor...</p></div>`);
            } else {
              ...
              ssrRenderList(__props.languagePacks, (pack) => {
                _push2(`<tr class="hover"${_scopeId}><td${_scopeId}><div class="text-sm font-medium text-base-content"${_scopeId}>${ssrInterpolate(pack.name)}</div><div class="text-base-content/70 text-sm"${_scopeId}>${ssrInterpolate(pack.description)}</div></td><td${_scopeId}><span class="badge badge-primary"${_scopeId}>${ssrInterpolate(getLanguageName(pack.language))}</span></td><td class="text-base-content/70 text-sm"${_scopeId}>${ssrInterpolate(pack.word_count)}</td><td${_scopeId}><div class="flex gap-2"${_scopeId}><a${ssrRenderAttr("href", _ctx.route("rendition.language-packs.words", pack.id))} class="link link-primary"${_scopeId}> Kelimeler </a>`);
                if (isLoggedIn.value) {
              ...
              createVNode("div", { class: "p-6" }, [
                createVNode("div", { class: "mb-6 flex items-center justify-between" }, [
                  createVNode("div", null, [
                    ...
                    createVNode("p", { class: "text-base-content/70 text-sm" }, "Toplam Paket Sayısı: " + toDisplayString(__props.languagePacks.length), 1)
                  ]),
                ...
                createVNode("div", { class: "card bg-base-100 shadow-lg" }, [
                  ...
                  isLoading.value ? (openBlock(), createBlock("div", {
                    ...
                    createVNode("p", { class: "text-base-content/70 mt-2" }, "Kelime listesi yükleniyor...")
                        ...
                        (openBlock(true), createBlock(Fragment, null, renderList(__props.languagePacks, (pack) => {
                          return openBlock(), createBlock("tr", {
                            ...
                            createVNode("td", null, [
                              ...
                              createVNode("div", { class: "text-base-content/70 text-sm" }, toDisplayString(pack.description), 1)
                            ]),
                            createVNode("td", null, [
                              createVNode("span", { class: "badge badge-primary" }, toDisplayString(getLanguageName(pack.language)), 1)
                            ]),
                            createVNode("td", { class: "text-base-content/70 text-sm" }, toDisplayString(pack.word_count), 1),
                            createVNode("td", null, [
