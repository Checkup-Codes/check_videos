import { computed, ref, withCtx, createVNode, openBlock, createBlock, toDisplayString, createCommentVNode, withDirectives, vModelText, vModelSelect, Fragment, renderList, createTextVNode, useSSRContext } from "vue";
            ...
            ssrRenderList(filteredWords.value, (word) => {
              ...
              if (isLoggedIn.value) {
                ...
                if (word == null ? void 0 : word.id) {
                  _push2(`<a${ssrRenderAttr("href", _ctx.route("rendition.words.edit", word.id))} class="rounded bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-700 hover:bg-yellow-200"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2v-5m-6-6l6 6M15 3l6 6"${_scopeId}></path></svg></a>`);
                } else {
                  _push2(`<!---->`);
                }
                if ((word == null ? void 0 : word.id) && (word == null ? void 0 : word.word)) {
                  _push2(`<button class="rounded bg-red-100 px-2 py-1 text-xs font-medium text-red-700 hover:bg-red-200"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5-4h4a1 1 0 011 1v1H9V4a1 1 0 011-1z"${_scopeId}></path></svg></button>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></td>`);
