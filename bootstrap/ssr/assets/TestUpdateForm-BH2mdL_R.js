import { ref, computed, onMounted, onUnmounted, watch, mergeProps, unref, withCtx, createTextVNode, useSSRContext } from "vue";
  ...
  setup(__props) {
    ...
    onMounted(() => {
      ...
      if (test.value.questions && test.value.questions.length > 0) {
        form.questions = test.value.questions.map((q) => {
          ...
          if (q.question_type === "true_false" && q.correct_answer !== void 0 && q.correct_answer !== null) {
          ...
          } else if (q.correct_answer !== void 0) {
    ...
    const validateForm = () => {
      ...
      form.questions.forEach((question, qIndex) => {
        ...
        if (question.question_type === "single_choice" || question.question_type === "multiple_choice") {
          ...
          if (question.correct_answer === null || question.correct_answer === void 0) {
    ...
    const updateTest = () => {
      ...
      form.put(route("tests.update", { test: test.value.slug }), {
        onSuccess: (page) => {
          var _a, _b;
          if (((_b = (_a = page == null ? void 0 : page.props) == null ? void 0 : _a.test) == null ? void 0 : _b.slug) || form.slug) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(form).questions, (question, questionIndex) => {
        ...
        _push(`</div><div class="grid grid-cols-1 gap-4 md:grid-cols-3"><div><label class="mb-1 block text-sm font-medium text-foreground">Soru Tipi *</label><select class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"><option value="single_choice"${ssrIncludeBooleanAttr(Array.isArray(question.question_type) ? ssrLooseContain(question.question_type, "single_choice") : ssrLooseEqual(question.question_type, "single_choice")) ? " selected" : ""}>Tek Doğru Cevap</option><option value="multiple_choice"${ssrIncludeBooleanAttr(Array.isArray(question.question_type) ? ssrLooseContain(question.question_type, "multiple_choice") : ssrLooseEqual(question.question_type, "multiple_choice")) ? " selected" : ""}>Birden Fazla Doğru Cevap</option><option value="true_false"${ssrIncludeBooleanAttr(Array.isArray(question.question_type) ? ssrLooseContain(question.question_type, "true_false") : ssrLooseEqual(question.question_type, "true_false")) ? " selected" : ""}>Doğru/Yanlış</option></select></div><div><label class="mb-1 block text-sm font-medium text-foreground">Puan</label><input type="number"${ssrRenderAttr("value", question.points)} min="1" placeholder="10" class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"></div><div><label class="mb-1 block text-sm font-medium text-foreground">Açıklama</label><input type="text"${ssrRenderAttr("value", question.explanation)} placeholder="Doğru cevap açıklaması (opsiyonel)" class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"></div></div>`);
        if (question.question_type === "single_choice") {
          _push(`<div class="space-y-3"><div class="flex items-center justify-between"><label class="block text-sm font-medium text-foreground">Şıklar * (Tek doğru cevap seçin)</label><button type="button" class="inline-flex h-8 items-center justify-center gap-1 rounded-md border border-input bg-background px-3 text-xs font-medium text-foreground hover:bg-accent"><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Şık Ekle </button></div><!--[-->`);
          ssrRenderList(question.options, (option, optionIndex) => {
            ...
            }, "flex-1 rounded-md border border-input bg-background px-3 py-1.5 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"])}"></div></div><div class="flex items-center gap-2"><label class="flex cursor-pointer items-center gap-2"><input type="radio"${ssrRenderAttr("name", `question-${questionIndex}-correct`)}${ssrIncludeBooleanAttr(option.is_correct) ? " checked" : ""} class="h-4 w-4 cursor-pointer border-2 border-muted-foreground/30 bg-background text-primary focus:ring-primary dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"><span class="text-xs text-muted-foreground">Doğru</span></label>`);
          ...
          _push(`<div class="space-y-3"><div class="flex items-center justify-between"><label class="block text-sm font-medium text-foreground">Şıklar * (Birden fazla doğru cevap seçebilirsiniz)</label><button type="button" class="inline-flex h-8 items-center justify-center gap-1 rounded-md border border-input bg-background px-3 text-xs font-medium text-foreground hover:bg-accent"><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Şık Ekle </button></div><!--[-->`);
          ssrRenderList(question.options, (option, optionIndex) => {
            _push(`<div class="flex items-start gap-3 rounded-lg border border-border bg-background p-3"><div class="flex-1 space-y-2"><div class="flex items-center gap-2"><span class="flex h-6 w-6 items-center justify-center rounded-full bg-muted text-xs font-medium text-muted-foreground">${ssrInterpolate(String.fromCharCode(65 + optionIndex))}</span><input type="text"${ssrRenderAttr("value", option.option_text)}${ssrRenderAttr("placeholder", `Şık ${optionIndex + 1} metni`)} class="${ssrRenderClass([{
              "border-destructive focus-visible:ring-destructive": errors.value[`questions.${questionIndex}.options.${optionIndex}.option_text`] || unref(form).errors[`questions.${questionIndex}.options.${optionIndex}.option_text`]
            }, "flex-1 rounded-md border border-input bg-background px-3 py-1.5 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"])}"></div></div><div class="flex items-center gap-2"><label class="flex cursor-pointer items-center gap-2"><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(option.is_correct) ? ssrLooseContain(option.is_correct, null) : option.is_correct) ? " checked" : ""} class="h-4 w-4 cursor-pointer rounded border-2 border-muted-foreground/30 bg-background text-primary focus:ring-primary dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"><span class="text-xs text-muted-foreground">Doğru</span></label>`);
            if (question.options.length > 2) {
              _push(`<button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md text-destructive hover:bg-destructive/10"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>`);
            } else {
              _push(`<!---->`);
          ...
          _push(`<div class="space-y-3"><label class="block text-sm font-medium text-foreground">Doğru Cevap *</label><div class="flex gap-4"><label class="${ssrRenderClass([{
          ...
          }, "flex cursor-pointer items-center gap-2 rounded-lg border-2 p-4 transition-all"])}"><input type="radio"${ssrRenderAttr("name", `question-${questionIndex}-tf`)}${ssrRenderAttr("value", true)}${ssrIncludeBooleanAttr(ssrLooseEqual(question.correct_answer, true)) ? " checked" : ""} class="h-4 w-4 cursor-pointer border-2 border-muted-foreground/30 bg-background text-primary focus:ring-primary dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"><span class="font-medium">Doğru</span></label><label class="${ssrRenderClass([{
          ...
          }, "flex cursor-pointer items-center gap-2 rounded-lg border-2 p-4 transition-all"])}"><input type="radio"${ssrRenderAttr("name", `question-${questionIndex}-tf`)}${ssrRenderAttr("value", false)}${ssrIncludeBooleanAttr(ssrLooseEqual(question.correct_answer, false)) ? " checked" : ""} class="h-4 w-4 cursor-pointer border-2 border-muted-foreground/30 bg-background text-primary focus:ring-primary dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"><span class="font-medium">Yanlış</span></label></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
