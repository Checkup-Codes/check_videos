  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.versions && __props.versions.length === 0) {
        _push(`<div class="flex h-32 items-center justify-center text-center opacity-50"><div>Henüz versiyon bulunmuyor</div></div>`);
      } else {
        ...
        ssrRenderList(__props.versions, (version) => {
          _push(ssrRenderComponent(unref(Link), {
            ...
            class: [
              ...
              getLinkClasses(`/versions/${version.version}`) ? "bg-base-content text-base-100" : "border border-base-300 bg-base-100 hover:bg-base-300"
            ]
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<div class="mb-1"${_scopeId}><h3 class="${ssrRenderClass([getLinkClasses(`/versions/${version.version}`) ? "text-base-100" : "text-base-content", "text-sm font-medium leading-tight"])}"${_scopeId}>${ssrInterpolate(version.version)}</h3></div><div class="${ssrRenderClass([getLinkClasses(`/versions/${version.version}`) ? "text-base-100/70" : "text-base-content/70", "flex items-center gap-3 text-xs"])}"${_scopeId}><span class="flex items-center gap-1"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"${_scopeId}></path></svg><span class="truncate"${_scopeId}>${ssrInterpolate(formatDate(version.release_date))}</span></span></div>`);
              } else {
                return [
                  createVNode("div", { class: "mb-1" }, [
                    createVNode("h3", {
                      class: ["text-sm font-medium leading-tight", getLinkClasses(`/versions/${version.version}`) ? "text-base-100" : "text-base-content"]
                    }, toDisplayString(version.version), 3)
                  ]),
                  createVNode("div", {
                    class: ["flex items-center gap-3 text-xs", getLinkClasses(`/versions/${version.version}`) ? "text-base-100/70" : "text-base-content/70"]
                  }, [
