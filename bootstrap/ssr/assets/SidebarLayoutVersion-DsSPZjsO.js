...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const getLinkClasses = (href) => {
      return url === href ? "block cursor-pointer text-sm rounded-lg text-black transition-all duration-200 bg-gray-900 text-white shadow-lg" : "block cursor-pointer text-sm rounded-lg text-black transition-all duration-200 hover:bg-gray-200 hover:shadow-lg";
    };
    onMounted(() => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="fixed z-30 mt-14 w-full shadow-lg shadow-color-one lg:mt-0 lg:w-[27%]"><div class="flex cursor-pointer justify-between text-sm text-black"><div class="flex justify-between p-2 text-sm font-bold text-black"><div class="rounded border-b-4 border-blue-100 p-2">Versiyonlar</div></div>`);
      if (unref(auth).user) {
        _push(`<div>`);
      ...
      ssrRenderList(versions.value, (version) => {
        ...
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="py-1 font-bold"${_scopeId}>${ssrInterpolate(version.version)}</div><div class="flex"${_scopeId}><div class="py-0.5 text-sm text-gray-400"${_scopeId}>${ssrInterpolate(formatDate(version.updated_at))}</div></div>`);
            } else {
              return [
