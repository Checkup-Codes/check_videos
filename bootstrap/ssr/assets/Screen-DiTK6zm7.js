...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto mt-10 w-full rounded-lg bg-white p-2 shadow-md lg:mt-0" }, _attrs))}><div class="flex items-center justify-between"><div class="hidden text-sm text-gray-500 lg:block">Kategori: ${ssrInterpolate(getCategoryName(write.value.category_id))}</div><div class="block lg:hidden">`);
      ...
      _push(`</div><div class=""><div class="p-4 lg:p-8"><h1 class="mb-6 text-3xl font-bold">${ssrInterpolate(write.value.title)}</h1><div class="prose mb-6">${write.value.content ?? ""}</div><div class="rounded-lg bg-gray-100 p-4"><h2 class="mb-2 text-xl font-semibold">Özet</h2><p>${ssrInterpolate(write.value.summary)}</p></div>`);
      if (unref(auth).user) {
