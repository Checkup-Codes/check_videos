import { computed, ref, onMounted, nextTick, onUnmounted, watch, mergeProps, withCtx, unref, createTextVNode, createVNode, toDisplayString, openBlock, createBlock, createCommentVNode, withModifiers, Fragment, renderList, withDirectives, vModelRadio, vModelCheckbox, vModelText, useSSRContext } from "vue";
import { ssrRenderComponent, ssrRenderClass, ssrInterpolate, ssrRenderStyle, ssrRenderList, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseEqual, ssrLooseContain } from "vue/server-renderer";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const getQuestionNavClasses = (questionId) => {
      ...
      if (isActive) {
        return isAnswered ? "bg-green-600 font-medium text-white shadow-sm hover:bg-green-600" : "bg-accent font-medium text-accent-foreground";
      }
      if (isAnswered) {
    ...
    const setupActiveQuestionTracking = () => {
      ...
      const scrollContainer = ((_a = questionElements[0]) == null ? void 0 : _a.closest(".overflow-y-auto")) || null;
      ...
      const answersArray = test.questions.map((question) => {
        ...
        if (question.question_type === "single_choice") {
          ...
          return {
            question_id: question.id,
            option_ids: null,
            answer_text: answer !== void 0 && answer !== null ? String(answer) : null
          };
        }
        return {
          question_id: question.id,
    ...
    onUnmounted(() => {
      ...
      if (scrollHandler) {
        const firstQuestion = ((_b = (_a = test.questions) == null ? void 0 : _a[0]) == null ? void 0 : _b.id) ? document.getElementById(`question-${test.questions[0].id}`) : null;
        const scrollContainer = (firstQuestion == null ? void 0 : firstQuestion.closest(".overflow-y-auto")) || null;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$1, mergeProps({ "info-class": "test-take-screen" }, _attrs), {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="${ssrRenderClass([{
              "xl:-translate-x-[100px]": showQuestionNavigation.value && isQuestionNavigationOpen.value
            }, "mx-auto min-h-full max-w-4xl space-y-6 p-4 pb-28 transition-all duration-300 lg:p-6 lg:pb-6"])}"${_scopeId}><div class="space-y-4 rounded-lg border border-border bg-card p-6"${_scopeId}><div class="flex items-center justify-between"${_scopeId}><h1 class="text-3xl font-bold text-foreground"${_scopeId}>${ssrInterpolate(unref(test).title)}</h1>`);
            ...
            if (unref(test).description) {
              _push2(`<div class="text-muted-foreground"${_scopeId}>${ssrInterpolate(unref(stripHtml)(unref(test).description))}</div>`);
            } else {
              _push2(`<!---->`);
            }
            ...
            ssrRenderList(unref(test).questions, (question, index) => {
              _push2(`<div${ssrRenderAttr("id", `question-${question.id}`)} class="scroll-mt-24 space-y-4 rounded-lg border border-border bg-card p-6"${_scopeId}><div${_scopeId}><div${_scopeId}><div class="mb-2 flex flex-wrap items-center gap-2"${_scopeId}><span class="rounded-md bg-primary px-2 py-1 text-sm font-semibold text-primary-foreground"${_scopeId}> Soru ${ssrInterpolate(index + 1)}</span><span class="text-sm text-muted-foreground"${_scopeId}>(${ssrInterpolate(question.points)} puan)</span><button type="button"${ssrIncludeBooleanAttr(!isQuestionAnswered(question.id)) ? " disabled" : ""}${ssrRenderAttr("aria-hidden", !isQuestionAnswered(question.id))} class="${ssrRenderClass([{ "invisible pointer-events-none": !isQuestionAnswered(question.id) }, "ml-auto inline-flex h-8 items-center justify-center rounded-md border border-border bg-background px-3 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"])}"${_scopeId}> Seçimi temizle </button></div><p class="whitespace-pre-wrap text-lg font-medium text-foreground"${_scopeId}>${ssrInterpolate(question.question_text)}</p></div></div><div class="space-y-2"${_scopeId}>`);
              if (question.question_type === "single_choice" || question.question_type === "multiple_choice" && hasOnlyOneCorrectAnswer(question)) {
                _push2(`<!--[-->`);
                ssrRenderList(question.options, (option, optIndex) => {
                  _push2(`<label${ssrRenderAttr("for", `question-${question.id}-option-${option.id}`)} class="${ssrRenderClass([{
                    "border-primary bg-primary/10 ring-2 ring-primary/20 dark:border-blue-400 dark:bg-blue-500/15 dark:ring-blue-400/20": answers.value[question.id] === option.id,
                    "border-border bg-background hover:border-primary/50 hover:bg-accent": answers.value[question.id] !== option.id
                  }, "flex cursor-pointer items-start gap-3 rounded-lg border-2 p-4 transition-all duration-200"])}"${_scopeId}><input${ssrRenderAttr("id", `question-${question.id}-option-${option.id}`)} type="radio"${ssrRenderAttr("name", `question-${question.id}`)}${ssrRenderAttr("value", option.id)}${ssrIncludeBooleanAttr(ssrLooseEqual(answers.value[question.id], option.id)) ? " checked" : ""} class="mt-1 h-5 w-5 cursor-pointer border-2 border-muted-foreground/30 text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"${_scopeId}><div class="flex-1"${_scopeId}><div class="flex items-center gap-2"${_scopeId}><span class="${ssrRenderClass([answers.value[question.id] === option.id ? "text-primary" : "text-muted-foreground", "font-semibold transition-colors"])}"${_scopeId}>${ssrInterpolate(String.fromCharCode(65 + optIndex))}. </span><span class="${ssrRenderClass([answers.value[question.id] === option.id ? "font-medium text-foreground" : "text-foreground", "whitespace-pre-wrap transition-colors"])}"${_scopeId}>${ssrInterpolate(option.option_text)}</span></div></div></label>`);
                });
                _push2(`<!--]-->`);
              } else if (question.question_type === "multiple_choice" && !hasOnlyOneCorrectAnswer(question)) {
                _push2(`<!--[-->`);
                ssrRenderList(question.options, (option, optIndex) => {
                  _push2(`<label${ssrRenderAttr("for", `question-${question.id}-option-${option.id}`)} class="${ssrRenderClass([{
                  ...
                  }, "flex cursor-pointer items-start gap-3 rounded-lg border-2 p-4 transition-all duration-200"])}"${_scopeId}><input${ssrRenderAttr("id", `question-${question.id}-option-${option.id}`)} type="checkbox"${ssrRenderAttr("value", option.id)}${ssrIncludeBooleanAttr(Array.isArray(answers.value[question.id]) ? ssrLooseContain(answers.value[question.id], option.id) : answers.value[question.id]) ? " checked" : ""} class="mt-1 h-5 w-5 cursor-pointer rounded border-2 border-muted-foreground/30 text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"${_scopeId}><div class="flex-1"${_scopeId}><div class="flex items-center gap-2"${_scopeId}><span class="${ssrRenderClass([isOptionSelected(question.id, option.id) ? "text-primary" : "text-muted-foreground", "font-semibold transition-colors"])}"${_scopeId}>${ssrInterpolate(String.fromCharCode(65 + optIndex))}. </span><span class="${ssrRenderClass([
                    isOptionSelected(question.id, option.id) ? "font-medium text-foreground" : "text-foreground",
                    "whitespace-pre-wrap transition-colors"
                  ])}"${_scopeId}>${ssrInterpolate(option.option_text)}</span></div></div></label>`);
                });
                _push2(`<!--]-->`);
              } else if (question.question_type === "true_false") {
                _push2(`<!--[--><label${ssrRenderAttr("for", `question-${question.id}-true`)} class="${ssrRenderClass([{
                  "border-primary bg-primary/10 ring-2 ring-primary/20 dark:border-blue-400 dark:bg-blue-500/15 dark:ring-blue-400/20": answers.value[question.id] === true,
                  "border-border bg-background hover:border-primary/50 hover:bg-accent": answers.value[question.id] !== true
                }, "flex cursor-pointer items-start gap-3 rounded-lg border-2 p-4 transition-all duration-200"])}"${_scopeId}><input${ssrRenderAttr("id", `question-${question.id}-true`)} type="radio"${ssrRenderAttr("name", `question-${question.id}`)}${ssrRenderAttr("value", true)}${ssrIncludeBooleanAttr(ssrLooseEqual(answers.value[question.id], true)) ? " checked" : ""} class="mt-1 h-5 w-5 cursor-pointer border-2 border-muted-foreground/30 text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"${_scopeId}><div class="flex-1"${_scopeId}><span class="${ssrRenderClass([answers.value[question.id] === true ? "text-primary" : "text-foreground", "whitespace-pre-wrap font-medium transition-colors"])}"${_scopeId}> Doğru </span></div></label><label${ssrRenderAttr("for", `question-${question.id}-false`)} class="${ssrRenderClass([{
                  "border-primary bg-primary/10 ring-2 ring-primary/20 dark:border-blue-400 dark:bg-blue-500/15 dark:ring-blue-400/20": answers.value[question.id] === false,
                  "border-border bg-background hover:border-primary/50 hover:bg-accent": answers.value[question.id] !== false
                }, "flex cursor-pointer items-start gap-3 rounded-lg border-2 p-4 transition-all duration-200"])}"${_scopeId}><input${ssrRenderAttr("id", `question-${question.id}-false`)} type="radio"${ssrRenderAttr("name", `question-${question.id}`)}${ssrRenderAttr("value", false)}${ssrIncludeBooleanAttr(ssrLooseEqual(answers.value[question.id], false)) ? " checked" : ""} class="mt-1 h-5 w-5 cursor-pointer border-2 border-muted-foreground/30 text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500"${_scopeId}><div class="flex-1"${_scopeId}><span class="${ssrRenderClass([answers.value[question.id] === false ? "text-primary" : "text-foreground", "whitespace-pre-wrap font-medium transition-colors"])}"${_scopeId}> Yanlış </span></div></label><!--]-->`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div></div>`);
            ...
            _push2(`<div class="fixed inset-x-0 bottom-0 z-40 flex flex-col gap-3 border-t border-border bg-background/95 px-4 py-3 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] backdrop-blur lg:static lg:flex-row lg:border-0 lg:bg-transparent lg:px-0 lg:py-0 lg:shadow-none lg:backdrop-blur-0" style="${ssrRenderStyle({ "padding-bottom": "max(0.75rem, env(safe-area-inset-bottom))" })}"${_scopeId}><button type="submit"${ssrIncludeBooleanAttr(unref(form).processing || !canSubmit.value) ? " disabled" : ""} class="inline-flex h-11 w-full items-center justify-center rounded-md bg-primary px-8 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50 lg:w-auto"${_scopeId}>`);
            ...
            if (showQuestionNavigation.value) {
              ...
              ssrRenderList(unref(test).questions, (question, index) => {
                _push2(`<button class="${ssrRenderClass([getQuestionNavClasses(question.id), "flex w-full items-center justify-between rounded-md px-3 py-2 text-sm transition-colors"])}"${_scopeId}><span class="font-medium"${_scopeId}>Soru ${ssrInterpolate(index + 1)}</span>`);
                if (isQuestionAnswered(question.id)) {
                  _push2(`<span class="text-[11px] font-medium"${_scopeId}>Cevaplandı</span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</button>`);
            ...
            if (showQuestionNavigation.value) {
              ...
              ssrRenderList(unref(test).questions, (question, index) => {
                _push2(`<button class="${ssrRenderClass([getQuestionNavClasses(question.id), "flex w-full items-center justify-between rounded-md px-2 py-1.5 text-xs transition-colors"])}"${_scopeId}><span class="font-medium"${_scopeId}>Soru ${ssrInterpolate(index + 1)}</span>`);
                if (isQuestionAnswered(question.id)) {
                  _push2(`<span class="text-[10px] font-medium"${_scopeId}>Cevaplandı</span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</button>`);
            ...
            return [
              createVNode("div", {
                ...
                createVNode("div", { class: "space-y-4 rounded-lg border border-border bg-card p-6" }, [
                  ...
                  }, toDisplayString(unref(stripHtml)(unref(test).description)), 1)) : createCommentVNode("", true),
                  ...
                  (openBlock(true), createBlock(Fragment, null, renderList(unref(test).questions, (question, index) => {
                    return openBlock(), createBlock("div", {
                      ...
                      createVNode("div", null, [
                        createVNode("div", null, [
                          createVNode("div", { class: "mb-2 flex flex-wrap items-center gap-2" }, [
                            ...
                            createVNode("button", {
                              ...
                              onClick: ($event) => clearQuestionAnswer(question),
                              class: ["ml-auto inline-flex h-8 items-center justify-center rounded-md border border-border bg-background px-3 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground", { "invisible pointer-events-none": !isQuestionAnswered(question.id) }]
                            }, " Seçimi temizle ", 10, ["disabled", "aria-hidden", "onClick"])
                          ]),
                          createVNode("p", { class: "whitespace-pre-wrap text-lg font-medium text-foreground" }, toDisplayString(question.question_text), 1)
                        ])
                      ]),
                      createVNode("div", { class: "space-y-2" }, [
                        question.question_type === "single_choice" || question.question_type === "multiple_choice" && hasOnlyOneCorrectAnswer(question) ? (openBlock(true), createBlock(Fragment, { key: 0 }, renderList(question.options, (option, optIndex) => {
                          return openBlock(), createBlock("label", {
                            ...
                            createVNode("div", { class: "flex-1" }, [
                              createVNode("div", { class: "flex items-center gap-2" }, [
                                createVNode("span", {
                                  class: ["font-semibold transition-colors", answers.value[question.id] === option.id ? "text-primary" : "text-muted-foreground"]
                                }, toDisplayString(String.fromCharCode(65 + optIndex)) + ". ", 3),
                                createVNode("span", {
                                  class: ["whitespace-pre-wrap transition-colors", answers.value[question.id] === option.id ? "font-medium text-foreground" : "text-foreground"]
                                }, toDisplayString(option.option_text), 3)
                              ])
                            ])
                          ], 10, ["for"]);
                        }), 128)) : question.question_type === "multiple_choice" && !hasOnlyOneCorrectAnswer(question) ? (openBlock(true), createBlock(Fragment, { key: 1 }, renderList(question.options, (option, optIndex) => {
                          return openBlock(), createBlock("label", {
                            key: option.id,
                            for: `question-${question.id}-option-${option.id}`,
                            class: ["flex cursor-pointer items-start gap-3 rounded-lg border-2 p-4 transition-all duration-200", {
                            ...
                            withDirectives(createVNode("input", {
                              ...
                              [vModelCheckbox, answers.value[question.id]]
                            ]),
                            createVNode("div", { class: "flex-1" }, [
                              createVNode("div", { class: "flex items-center gap-2" }, [
                                createVNode("span", {
                                  class: ["font-semibold transition-colors", isOptionSelected(question.id, option.id) ? "text-primary" : "text-muted-foreground"]
                                }, toDisplayString(String.fromCharCode(65 + optIndex)) + ". ", 3),
                                createVNode("span", {
                                  class: [
                                    "whitespace-pre-wrap transition-colors",
                        ...
                        }), 128)) : question.question_type === "true_false" ? (openBlock(), createBlock(Fragment, { key: 2 }, [
                          createVNode("label", {
                            ...
                            withDirectives(createVNode("input", {
                              ...
                              class: "mt-1 h-5 w-5 cursor-pointer border-2 border-muted-foreground/30 text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:border-muted-foreground/60 dark:bg-background dark:checked:border-blue-400 dark:checked:bg-blue-500",
                              onChange: updateAnsweredCount
                            }, null, 40, ["id", "name", "onUpdate:modelValue"]), [
                              [vModelRadio, answers.value[question.id]]
                            ]),
                            createVNode("div", { class: "flex-1" }, [
                              createVNode("span", {
                                class: ["whitespace-pre-wrap font-medium transition-colors", answers.value[question.id] === true ? "text-primary" : "text-foreground"]
                              }, " Doğru ", 2)
                            ])
                          ], 10, ["for"]),
                          createVNode("label", {
                            ...
                            createVNode("div", { class: "flex-1" }, [
                              createVNode("span", {
                                class: ["whitespace-pre-wrap font-medium transition-colors", answers.value[question.id] === false ? "text-primary" : "text-foreground"]
                              }, " Yanlış ", 2)
                            ])
                          ], 10, ["for"])
                    ...
                    createVNode("nav", { class: "space-y-1" }, [
                      (openBlock(true), createBlock(Fragment, null, renderList(unref(test).questions, (question, index) => {
                        return openBlock(), createBlock("button", {
                          ...
                          class: ["flex w-full items-center justify-between rounded-md px-3 py-2 text-sm transition-colors", getQuestionNavClasses(question.id)]
                        }, [
                          createVNode("span", { class: "font-medium" }, "Soru " + toDisplayString(index + 1), 1),
                          isQuestionAnswered(question.id) ? (openBlock(), createBlock("span", {
                            key: 0,
                            class: "text-[11px] font-medium"
                          }, "Cevaplandı")) : createCommentVNode("", true)
                        ], 10, ["onClick"]);
                      }), 128))
                    ])
                  ], 512)
                    ...
                    createVNode("nav", { class: "space-y-1" }, [
                      (openBlock(true), createBlock(Fragment, null, renderList(unref(test).questions, (question, index) => {
                        return openBlock(), createBlock("button", {
                          ...
                          ref: (el) => setQuestionNavItemRef(el, question.id),
                          onClick: ($event) => scrollToQuestion(question.id),
                          class: ["flex w-full items-center justify-between rounded-md px-2 py-1.5 text-xs transition-colors", getQuestionNavClasses(question.id)]
                        }, [
                          createVNode("span", { class: "font-medium" }, "Soru " + toDisplayString(index + 1), 1),
                          isQuestionAnswered(question.id) ? (openBlock(), createBlock("span", {
                            key: 0,
