...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const isAnswerBlank = (answer) => {
      ...
      const hasTextAnswer = answer.answer_text !== null && answer.answer_text !== void 0 && answer.answer_text !== "";
...
`;
      ...
      summary += `
Tekrar önerisi: `;
      summary += wrongAnsweredQuestions.length || blankAnswerCount ? `${wrongAnsweredQuestions.length} yanlış ve ${blankAnswerCount} boş soruyu açıklamalarıyla tekrar et ve benzer sorularla pekiştir.` : `Tüm sorular doğru. Bilgiyi kalıcı yapmak için testi daha sonra tekrar çöz.`;
      ...
      _push(ssrRenderComponent(_sfc_main$1, mergeProps({ infoClass: "test-result-screen" }, _attrs), {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (_push2) {
            ...
            _push2(`<!--]--></div></section>`);
            if (blankAnswers.value.length) {
              _push2(`<section class="rounded-lg border border-amber-200 bg-amber-50/70 p-4 dark:border-amber-900/70 dark:bg-amber-950/30 sm:p-5"${_scopeId}><div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"${_scopeId}><h2 class="text-base font-semibold text-foreground"${_scopeId}>Boş bırakılan sorular</h2><span class="text-xs text-muted-foreground"${_scopeId}>${ssrInterpolate(blankAnswers.value.length)} soru</span></div><div class="mt-3 grid gap-2 lg:grid-cols-2"${_scopeId}><!--[-->`);
              ssrRenderList(blankAnswers.value, (blankAnswer) => {
                var _a2;
                _push2(`<div class="rounded-md border border-amber-200/80 bg-background p-3 dark:border-amber-900/60"${_scopeId}><p class="whitespace-pre-wrap text-sm leading-5 text-foreground"${_scopeId}> Soru ${ssrInterpolate(getAnswerNumber(blankAnswer))}: ${ssrInterpolate((_a2 = blankAnswer.question) == null ? void 0 : _a2.question_text)}</p></div>`);
              });
              _push2(`<!--]--></div></section>`);
            } else {
              _push2(`<!---->`);
            ...
            ssrRenderList(resultAnswers.value, (answer, index) => {
              ...
              if (isAnswerBlank(answer)) {
                _push2(`<span class="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 dark:bg-amber-950 dark:text-amber-300"${_scopeId}> Boş </span>`);
              ...
              if (getQuestionOptions(answer.question).length > 0) {
                ...
                _push2(`<div class="rounded-md border border-dashed border-amber-200 bg-amber-50/70 p-3 dark:border-amber-900/70 dark:bg-amber-950/30"${_scopeId}><p class="text-sm font-medium text-amber-800 dark:text-amber-200"${_scopeId}>Bu soru boş bırakıldı.</p></div>`);
              }
            ...
            return [
              createVNode("div", { class: "mx-auto max-w-5xl space-y-5 px-3 py-4 pb-24 sm:px-4 sm:py-6 lg:pb-10" }, [
                  ...
                  createVNode("div", { class: "flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between" }, [
                    ...
                    createVNode("div", { class: "flex items-center gap-4 rounded-md border border-border bg-background p-3" }, [
                      ...
                      createVNode("div", null, [
                        ...
                        createVNode("p", { class: "mt-1 text-xs text-muted-foreground" }, toDisplayString(unref(result).correct_answers || 0) + " doğru, " + toDisplayString(wrongAnswers.value) + " yanlış, " + toDisplayString(blankAnswers.value.length) + " boş ", 1)
                      ])
                    ])
                  ]),
                  createVNode("div", { class: "mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5" }, [
                ...
                blankAnswers.value.length ? (openBlock(), createBlock("section", {
                  key: 0,
                  class: "rounded-lg border border-amber-200 bg-amber-50/70 p-4 dark:border-amber-900/70 dark:bg-amber-950/30 sm:p-5"
                }, [
                  createVNode("div", { class: "flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between" }, [
                    createVNode("h2", { class: "text-base font-semibold text-foreground" }, "Boş bırakılan sorular"),
                    createVNode("span", { class: "text-xs text-muted-foreground" }, toDisplayString(blankAnswers.value.length) + " soru", 1)
                  ]),
                  createVNode("div", { class: "mt-3 grid gap-2 lg:grid-cols-2" }, [
                    (openBlock(true), createBlock(Fragment, null, renderList(blankAnswers.value, (blankAnswer) => {
                      var _a2;
                      return openBlock(), createBlock("div", {
                        key: blankAnswer.id,
                        class: "rounded-md border border-amber-200/80 bg-background p-3 dark:border-amber-900/60"
                      }, [
                        createVNode("p", { class: "whitespace-pre-wrap text-sm leading-5 text-foreground" }, " Soru " + toDisplayString(getAnswerNumber(blankAnswer)) + ": " + toDisplayString((_a2 = blankAnswer.question) == null ? void 0 : _a2.question_text), 1)
                      ]);
                    }), 128))
                  ])
                    ...
                    (openBlock(true), createBlock(Fragment, null, renderList(resultAnswers.value, (answer, index) => {
                      ...
                      return openBlock(), createBlock("div", {
                        ...
                        getQuestionOptions(answer.question).length > 0 ? (openBlock(), createBlock("div", {
                          ...
                          createVNode("p", { class: "text-sm font-medium text-amber-800 dark:text-amber-200" }, "Bu soru boş bırakıldı.")
                        ])),
