        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<h1 class="${ssrRenderClass([
              ...
              category.value.name > 30 ? "text-xl" : "text-2xl"
            ])}"${_scopeId}><span${_scopeId}> Kategoriler: ${ssrInterpolate(category.value.name)}</span>`);
            _push2(ssrRenderComponent(_sfc_main$2, { onClick: editCategory }, {
            ...
            _push2(`</h1><div class="h-[calc(84vh)] w-full max-w-full overflow-y-scroll rounded-lg bg-white p-5"${_scopeId}>`);
            if (isGridView.value) {
            ...
            return [
              createVNode("h1", {
                onClick: _ctx.toggleContent,
                class: [
                  "group flex h-16 cursor-pointer select-none items-center justify-between border-b border-gray-300 bg-gradient-to-r from-gray-100 to-gray-200 px-4 py-4 font-extrabold text-gray-900 transition-all duration-300 ease-in-out hover:bg-gradient-to-l hover:shadow-md",
                  category.value.name > 30 ? "text-xl" : "text-2xl"
                ]
              }, [
                createVNode("span", null, " Kategoriler: " + toDisplayString(category.value.name), 1),
                createVNode(_sfc_main$2, { onClick: editCategory }, {
                  default: withCtx(() => [
                    createTextVNode(" Kategoriyi düzenle ")
                  ]),
                  _: 1
