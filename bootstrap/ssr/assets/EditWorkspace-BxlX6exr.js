    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$1, null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="space-y-6 py-6"${_scopeId}><div${_scopeId}><h1 class="text-2xl font-bold text-foreground"${_scopeId}>Çalışma Alanını Düzenle</h1><p class="mt-1 text-sm text-muted-foreground"${_scopeId}>Çalışma alanınızı ve ürünlerinizi güncelleyin</p></div><form class="space-y-6"${_scopeId}><div class="rounded-xl bg-card p-6 ring-1 ring-border/50"${_scopeId}><h2 class="mb-4 text-lg font-semibold text-foreground"${_scopeId}>Görseller</h2><div class="space-y-4"${_scopeId}>`);
            if (currentImages.value.length > 0 && imagePreviews.value.length === 0) {
              _push2(`<div${_scopeId}><p class="mb-2 text-sm text-muted-foreground"${_scopeId}>Mevcut Görseller</p><div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4"${_scopeId}><!--[-->`);
              ssrRenderList(currentImages.value, (image, index) => {
                _push2(`<div class="aspect-video overflow-hidden rounded-lg ring-1 ring-border"${_scopeId}><img${ssrRenderAttr("src", `/storage/${image}`)} alt="Current" class="h-full w-full object-cover"${_scopeId}></div>`);
              });
              _push2(`<!--]--></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<label class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-border px-6 py-8 transition-colors hover:border-primary"${_scopeId}><svg class="mb-2 h-10 w-10 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"${_scopeId}></path></svg><span class="text-sm font-medium text-foreground"${_scopeId}>${ssrInterpolate(currentImages.value.length > 0 ? "Resimleri Değiştir" : "Resim Yükle")}</span><span class="mt-1 text-xs text-muted-foreground"${_scopeId}>PNG, JPG, WEBP - max 5MB</span><input type="file" accept="image/*" multiple class="hidden"${_scopeId}></label>`);
            ...
            return [
              createVNode("div", { class: "space-y-6 py-6" }, [
                createVNode("div", null, [
                  createVNode("h1", { class: "text-2xl font-bold text-foreground" }, "Çalışma Alanını Düzenle"),
                  createVNode("p", { class: "mt-1 text-sm text-muted-foreground" }, "Çalışma alanınızı ve ürünlerinizi güncelleyin")
                ]),
                createVNode("form", {
