...
import { _ as _sfc_main$2 } from "./ExcalidrawComponent-D2-UY8xq.js";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="grid grid-cols-12 lg:px-10 lg:pt-5"><div class="col-span-9 my-auto"><h1 class="text-3xl font-bold">${ssrInterpolate(write.value.title)}</h1><div class="hidden text-sm text-gray-500 lg:block">Kategori: ${ssrInterpolate(getCategoryName(write.value.category_id))}</div></div><div class="col-span-3 items-end justify-end">`);
      if (write.value.hasDraw) {
        _push(`<div class="flex justify-end"><button class="rounded-md border-2 px-3 py-1 text-black shadow-md shadow-blue-200">${ssrInterpolate(showMerhaba.value ? "Yazıya Dön" : "Drawina git")}</button></div>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(auth).user) {
        ...
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="rounded-md border-2 px-3 py-1 text-black shadow-md shadow-blue-200"${_scopeId}>Yazıyı Düzenle</div>`);
            } else {
              return [
                createVNode("div", { class: "rounded-md border-2 px-3 py-1 text-black shadow-md shadow-blue-200" }, "Yazıyı Düzenle")
              ];
            }
          }),
          _: 1
