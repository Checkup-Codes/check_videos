import { computed, ref, mergeProps, withCtx, unref, createTextVNode, createVNode, withModifiers, withDirectives, vModelText, openBlock, createBlock, toDisplayString, createCommentVNode, Fragment, renderList, vModelSelect, vModelCheckbox, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const toggleTodo = (serviceId, index) => {
      var _a;
      const todo = (_a = servicesTodos.value[serviceId]) == null ? void 0 : _a[index];
      if (!todo) return;
    ...
    const handleSubmit = () => {
      ...
      form.transform((data) => ({
        ...
        services: selectedServices.value.map((serviceId) => ({
          ...getServiceData(serviceId),
          todos: getServiceTodos(serviceId).filter((todo) => todo.title && todo.title.trim()).map((todo) => ({
      ...
      _push(ssrRenderComponent(_sfc_main$1, mergeProps({
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<p class="mt-1 text-xs text-muted-foreground"${_scopeId}> Bu projeye ait yazıların bulunduğu kategoriyi seçin. </p></div></div><div class="space-y-4 border-t border-border pt-4"${_scopeId}><h3 class="text-xs font-semibold text-foreground"${_scopeId}>Hizmetler</h3>`);
            ...
            _push2(` ${ssrInterpolate(unref(form).processing ? "Kaydediliyor..." : "Projeyi Kaydet")}</button></div></div></form>`);
          } else {
            return [
              createVNode("form", {
                ...
                createVNode("div", { class: "rounded-lg border border-border bg-card p-5 shadow-sm" }, [
                  ...
                  createVNode("div", { class: "space-y-4 border-t border-border pt-4" }, [
                  ...
                  createVNode("div", { class: "flex justify-end gap-2 border-t border-border pt-4" }, [
                    ...
                    createVNode("button", {
                      ...
                      createTextVNode(" " + toDisplayString(unref(form).processing ? "Kaydediliyor..." : "Projeyi Kaydet"), 1)
                    ], 8, ["disabled"])
                  ])
                ])
              ], 32)
