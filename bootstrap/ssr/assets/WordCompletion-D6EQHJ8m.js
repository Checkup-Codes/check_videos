...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex flex-col items-center justify-center" }, _attrs))} data-v-1b7eec00>`);
      if (!gameState.value.isPlaying && gameState.value.userResponses.length > 0) {
        _push(`<div class="game-summary w-full max-w-md rounded-2xl border border-gray-200 bg-white px-6 shadow-sm" data-v-1b7eec00><h2 class="mb-2 text-2xl font-semibold text-gray-900" data-v-1b7eec00>Test Özeti</h2><div class="mb-4 text-center" data-v-1b7eec00><p class="text-4xl font-bold text-gray-900" data-v-1b7eec00>${ssrInterpolate(calculateScore.value)} / 100</p><p class="text-sm text-gray-600" data-v-1b7eec00>Puan</p></div><div class="mb-4 grid grid-cols-2 gap-4 rounded-lg bg-gray-50 p-4" data-v-1b7eec00><div class="text-center" data-v-1b7eec00><p class="text-2xl font-bold text-green-600" data-v-1b7eec00>${ssrInterpolate(correctCount.value)}</p><p class="text-sm text-gray-600" data-v-1b7eec00>Doğru</p></div><div class="text-center" data-v-1b7eec00><p class="text-2xl font-bold text-red-600" data-v-1b7eec00>${ssrInterpolate(incorrectCount.value)}</p><p class="text-sm text-gray-600" data-v-1b7eec00>Yanlış</p></div></div><div class="mb-4 text-center" data-v-1b7eec00><p class="text-3xl font-bold text-blue-600" data-v-1b7eec00>${ssrInterpolate(gameState.value.totalPoints)}</p><p class="text-sm text-gray-600" data-v-1b7eec00>Toplam Puan</p></div><div class="mb-4 max-h-60 overflow-y-auto" data-v-1b7eec00><h3 class="mb-2 font-medium text-gray-700" data-v-1b7eec00>Cevaplarınız:</h3><ul class="space-y-2 text-left text-sm text-gray-800" data-v-1b7eec00><!--[-->`);
        ssrRenderList(gameState.value.userResponses, (response) => {
          var _a, _b;
          _push(`<li class="flex items-center justify-between rounded-md bg-gray-50 px-4 py-2" data-v-1b7eec00><div class="flex flex-col" data-v-1b7eec00><span class="${ssrRenderClass(response.correct ? "font-medium text-green-600" : "font-medium text-red-600")}" data-v-1b7eec00>${ssrInterpolate(((_a = wordsMap.value[response.word_id]) == null ? void 0 : _a.word) || "Unknown")}</span><span class="text-xs text-gray-500" data-v-1b7eec00>${ssrInterpolate(((_b = wordsMap.value[response.word_id]) == null ? void 0 : _b.meaning) || "")}</span>`);
          if (response.correct) {
            _push(`<span class="text-xs text-blue-600" data-v-1b7eec00>${ssrInterpolate(response.points)} puan ${ssrInterpolate(response.hintsUsed > 0 ? `(${response.hintsUsed} ipucu kullanıldı)` : "")}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div><span class="${ssrRenderClass(response.correct ? "text-lg text-green-600" : "text-lg text-red-600")}" data-v-1b7eec00>${ssrInterpolate(response.correct ? "✓" : "✗")}</span></li>`);
        });
        _push(`<!--]--></ul></div><div class="space-y-2" data-v-1b7eec00>`);
        if (hasUser.value && !isUpdating.value) {
          _push(`<button class="w-full rounded-lg bg-blue-600 px-4 py-2 text-center text-sm font-medium text-white transition hover:bg-blue-700" data-v-1b7eec00> Kelime İstatistiklerini Güncelle ve Pakete Dön </button>`);
        } else {
          _push(`<!---->`);
        }
        if (hasUser.value && isUpdating.value) {
          _push(`<button disabled class="w-full rounded-lg bg-blue-400 px-4 py-2 text-center text-sm font-medium text-white" data-v-1b7eec00> Güncelleniyor... </button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="flex gap-2" data-v-1b7eec00><button class="flex-1 rounded-lg bg-black px-4 py-2 text-center text-sm font-medium text-white transition hover:bg-gray-700" data-v-1b7eec00> Pakete Dön </button><button class="flex-1 rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100" data-v-1b7eec00> Tekrar Başla </button></div></div></div>`);
      } else {
        ...
        }, "relative w-full max-w-md rounded-2xl border border-gray-200 p-6 shadow-sm transition-colors duration-500"])}" data-v-1b7eec00><div class="relative mb-4 h-1 w-full overflow-hidden rounded-full bg-gray-200" data-v-1b7eec00><div class="h-full w-0 bg-black" data-v-1b7eec00></div><div class="absolute left-0 top-0 h-full w-[50px] bg-gradient-to-r from-transparent via-white/30 to-transparent" data-v-1b7eec00></div></div><div class="mb-4 flex items-center justify-between text-sm text-gray-500" data-v-1b7eec00><div data-v-1b7eec00>Soru ${ssrInterpolate(gameState.value.currentIndex + 1)} / ${ssrInterpolate(gameState.value.totalQuestions)}</div><div class="flex flex-col items-end gap-1" data-v-1b7eec00><div class="flex items-center gap-2" data-v-1b7eec00><span class="font-medium text-blue-600" data-v-1b7eec00>${ssrInterpolate(gameState.value.totalPoints)}</span><span data-v-1b7eec00>toplam puan</span></div><div class="flex items-center gap-2 text-xs" data-v-1b7eec00><span class="font-medium text-gray-600" data-v-1b7eec00>${ssrInterpolate(currentQuestionPoints.value)}</span><span data-v-1b7eec00>mevcut soru puanı</span></div></div></div>`);
        if (gameState.value.currentQuestion) {
          _push(`<div class="space-y-4" data-v-1b7eec00><div data-v-1b7eec00><div class="flex items-center justify-between" data-v-1b7eec00><h2 class="text-xl font-semibold text-gray-900" data-v-1b7eec00>Eksik harfleri tamamlayın:</h2></div><p class="mt-1 text-sm text-gray-500" data-v-1b7eec00>Anlam: ${ssrInterpolate(gameState.value.currentQuestion.meaning)}</p></div><div class="space-y-2" data-v-1b7eec00><div class="flex justify-center space-x-2" data-v-1b7eec00><!--[-->`);
          ssrRenderList(gameState.value.currentQuestion.word.split(""), (letter, index) => {
            ...
            }, "flex h-12 w-12 items-center justify-center rounded-lg border text-xl font-semibold transition-all"])}" data-v-1b7eec00>`);
            if (gameState.value.maskedIndices.includes(index)) {
              _push(`<span class="${ssrRenderClass({ "text-blue-600": gameState.value.hintLetterIndices.includes(index) })}" data-v-1b7eec00>${ssrInterpolate(gameState.value.selectedLetters[index] || "")}</span>`);
            } else {
          ...
          if (gameState.value.showAnswer && !gameState.value.isCorrect) {
            _push(`<div class="flex justify-center space-x-2" data-v-1b7eec00><!--[-->`);
            ssrRenderList(gameState.value.currentQuestion.word.split(""), (letter, index) => {
              _push(`<div class="flex h-8 w-8 items-center justify-center rounded-lg border border-green-500 bg-green-50 text-sm font-medium text-green-700" data-v-1b7eec00>${ssrInterpolate(letter.toLowerCase())}</div>`);
            });
            _push(`<!--]--></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div><button${ssrIncludeBooleanAttr(gameState.value.showAnswer || !isAnswerComplete.value) ? " disabled" : ""} class="w-full rounded-lg bg-black px-4 py-2 text-center text-sm font-medium text-white transition hover:bg-gray-700 disabled:opacity-50" data-v-1b7eec00> Kontrol Et </button></div>`);
        } else {
          _push(`<!---->`);
        }
        if (showVirtualKeyboard.value) {
          _push(`<div class="mt-4" data-v-1b7eec00><div class="flex flex-col gap-1 rounded-lg border border-gray-200 bg-gray-100 p-2" data-v-1b7eec00><div class="flex gap-1" data-v-1b7eec00><!--[-->`);
          ssrRenderList(["q", "w", "e", "r", "t", "y", "u", "ı", "o", "p", "ğ", "ü"], (letter) => {
            _push(`<button class="flex h-10 w-10 items-center justify-center rounded-lg bg-white text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 active:bg-gray-100" data-v-1b7eec00>${ssrInterpolate(letter)}</button>`);
          });
          _push(`<!--]--></div><div class="flex gap-1" data-v-1b7eec00><!--[-->`);
          ...
          _push(`<!--]--></div><div class="flex gap-1" data-v-1b7eec00><button class="flex h-10 flex-1 items-center justify-center rounded-lg bg-white text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 active:bg-gray-100" data-v-1b7eec00> ← </button><button class="flex h-10 flex-1 items-center justify-center rounded-lg bg-blue-100 text-sm font-medium text-blue-700 shadow-sm transition hover:bg-blue-200 active:bg-blue-300" data-v-1b7eec00> İpucu </button></div></div></div>`);
        } else {
          _push(`<!---->`);
        }
        if (!showVirtualKeyboard.value) {
          _push(`<button class="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50" data-v-1b7eec00><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" data-v-1b7eec00><path d="M10 2a8 8 0 100 16 8 8 0 000-16zm0 14a6 6 0 110-12 6 6 0 010 12zm-1-5a1 1 0 011-1h2a1 1 0 110 2h-2a1 1 0 01-1-1zm0-4a1 1 0 011-1h2a1 1 0 110 2h-2a1 1 0 01-1-1z" data-v-1b7eec00></path></svg> Klavyeyi Aç </button>`);
        } else {
          _push(`<button class="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50" data-v-1b7eec00><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" data-v-1b7eec00><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" data-v-1b7eec00></path></svg> Klavyeyi Kapat </button>`);
        }
        _push(`</div>`);
      }
      _push(`</div>`);
...
const WordCompletion = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-1b7eec00"]]);
export {
