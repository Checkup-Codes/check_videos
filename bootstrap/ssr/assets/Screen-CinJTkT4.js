...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto mt-10 w-full max-w-full overflow-auto rounded-lg bg-screen-bg p-2 shadow-md lg:mt-0" }, _attrs))}><div class="min-h-screen"><div class="min-h-screen">`);
      if (unref(auth).user) {
        _push(`<div>`);
      ...
      _push(`<header class="flex h-screen items-center justify-center bg-screen-bg text-black"><div class="text-center"><h2 class="mb-4 text-4xl font-bold">Kategoriler : ${ssrInterpolate(category.value.name)}</h2></div></header></div></div></div>`);
    };
  }
};
