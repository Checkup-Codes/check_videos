    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!gameState.value.isPlaying && gameState.value.userResponses.length > 0) {
        ...
        ssrRenderList(gameState.value.userResponses, (response) => {
          var _a2, _b;
          _push(`<li class="flex items-center justify-between rounded-md bg-gray-50 px-4 py-2"><div class="flex flex-col"><span class="${ssrRenderClass(response.correct ? "font-medium text-green-600" : "font-medium text-red-600")}">${ssrInterpolate(((_a2 = wordsMap.value[response.word_id]) == null ? void 0 : _a2.word) || "Unknown")}</span><span class="text-xs text-gray-500">${ssrInterpolate(((_b = wordsMap.value[response.word_id]) == null ? void 0 : _b.meaning) || "")}</span></div><span class="${ssrRenderClass(response.correct ? "text-lg text-green-600" : "text-lg text-red-600")}">${ssrInterpolate(response.correct ? "✓" : "✗")}</span></li>`);
        });
        ...
        if (gameState.value.currentQuestion) {
          ...
          ssrRenderList(shuffledOptions.value, (option, index) => {
            ...
            _push(`<button${ssrIncludeBooleanAttr(gameState.value.showAnswer) ? " disabled" : ""} class="${ssrRenderClass([{
              "border-green-500 bg-green-50": gameState.value.showAnswer && option.meaning === gameState.value.currentQuestion.meaning,
              "border-red-500 bg-red-50": gameState.value.showAnswer && option.meaning !== gameState.value.currentQuestion.meaning && ((_a2 = gameState.value.selectedAnswer) == null ? void 0 : _a2.meaning) === option.meaning
            }, "w-full rounded-lg border border-gray-200 p-3 text-left text-base text-gray-900 transition hover:bg-gray-50 disabled:cursor-not-allowed"])}">${ssrInterpolate(option.meaning)}</button>`);
          });
          _push(`<!--]--></div></div>`);
        } else {
          _push(`<!---->`);
