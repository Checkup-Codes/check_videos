...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto mt-10 w-full max-w-full overflow-auto bg-screen-bg px-5 lg:mt-0" }, _attrs))}><div class="container mx-auto p-4"><h1 class="mb-6 text-3xl font-bold">Yazıyı Güncelle</h1>`);
      _push(ssrRenderComponent(_sfc_main$1, { onSubmit: updateWrite }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`</div><div class="mb-4"${_scopeId}><label for="status" class="mb-1 block text-sm font-bold"${_scopeId}>Durum:</label><select id="status" class="mt-1 block w-full"${_scopeId}><option value="draft"${ssrIncludeBooleanAttr(Array.isArray(unref(form).status) ? ssrLooseContain(unref(form).status, "draft") : ssrLooseEqual(unref(form).status, "draft")) ? " selected" : ""}${_scopeId}>Şablon</option><option value="published"${ssrIncludeBooleanAttr(Array.isArray(unref(form).status) ? ssrLooseContain(unref(form).status, "published") : ssrLooseEqual(unref(form).status, "published")) ? " selected" : ""}${_scopeId}>Yayında</option></select>`);
