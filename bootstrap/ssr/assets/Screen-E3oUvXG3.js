import { computed, ref, withCtx, unref, createVNode, createBlock, createCommentVNode, toDisplayString, openBlock, createTextVNode, Fragment, renderList, withModifiers, withDirectives, vModelText, vModelSelect, withKeys, nextTick, useSSRContext } from "vue";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="space-y-6 py-6"${_scopeId}><div${_scopeId}><h1 class="text-2xl font-bold text-foreground"${_scopeId}>Yeni Kelime</h1><p class="mt-1 text-sm text-muted-foreground"${_scopeId}>Kelime ekleyin, detayları sonra tamamlayabilirsiniz</p></div>`);
            if (__props.incompleteWords.length > 0) {
              _push2(`<div class="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-4"${_scopeId}><div class="flex items-start gap-3"${_scopeId}><svg class="mt-0.5 h-5 w-5 flex-shrink-0 text-yellow-600 dark:text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"${_scopeId}></path></svg><div class="flex-1"${_scopeId}><h3 class="font-medium text-yellow-800 dark:text-yellow-200"${_scopeId}>${ssrInterpolate(__props.incompleteWords.length)} yarım kalan kelime var </h3><p class="mt-1 text-sm text-yellow-700 dark:text-yellow-300"${_scopeId}> Bu kelimelerin anlamları henüz girilmemiş. </p><button class="mt-2 text-sm font-medium text-yellow-800 underline hover:no-underline dark:text-yellow-200"${_scopeId}>${ssrInterpolate(showIncompleteList.value ? "Gizle" : "Tamamla")}</button></div></div>`);
            ...
            _push2(`<button type="submit"${ssrIncludeBooleanAttr(processing.value || !unref(form).word.trim()) ? " disabled" : ""} class="rounded-lg bg-primary px-6 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50"${_scopeId}>${ssrInterpolate(processing.value ? "Kaydediliyor..." : "Kaydet")}</button></div></form></div>`);
          } else {
            return [
              createVNode("div", { class: "space-y-6 py-6" }, [
                createVNode("div", null, [
                  createVNode("h1", { class: "text-2xl font-bold text-foreground" }, "Yeni Kelime"),
                  createVNode("p", { class: "mt-1 text-sm text-muted-foreground" }, "Kelime ekleyin, detayları sonra tamamlayabilirsiniz")
                ]),
                __props.incompleteWords.length > 0 ? (openBlock(), createBlock("div", {
                  key: 0,
                  ...
                  createVNode("div", { class: "flex items-start gap-3" }, [
                    ...
                    createVNode("div", { class: "flex-1" }, [
                      createVNode("h3", { class: "font-medium text-yellow-800 dark:text-yellow-200" }, toDisplayString(__props.incompleteWords.length) + " yarım kalan kelime var ", 1),
