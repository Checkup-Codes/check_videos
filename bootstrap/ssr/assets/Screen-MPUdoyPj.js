import { computed, ref, withCtx, unref, createTextVNode, createVNode, toDisplayString, withModifiers, withDirectives, vModelText, openBlock, createBlock, createCommentVNode, Fragment, renderList, vModelSelect, useSSRContext } from "vue";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="mx-auto max-w-4xl"${_scopeId}><div class="mb-6"${_scopeId}><h1 class="text-xl font-semibold text-foreground"${_scopeId}>Hizmeti Düzenle</h1><p class="mt-1 text-xs text-muted-foreground"${_scopeId}>${ssrInterpolate(service.value.name)}</p></div><form class="space-y-4"${_scopeId}><div class="rounded-lg border border-border bg-card p-5 shadow-sm"${_scopeId}><div class="space-y-4"${_scopeId}><div${_scopeId}><label class="mb-1.5 block text-xs font-medium text-foreground"${_scopeId}>Hizmet Adı</label><input${ssrRenderAttr("value", unref(form).name)} type="text" placeholder="Örn: Web Geliştirme" class="${ssrRenderClass([{ "border-destructive focus-visible:ring-destructive": errors.value.name || unref(form).errors.name }, "flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"])}" required${_scopeId}>`);
            ...
            _push2(`</div><div${_scopeId}><label class="mb-1.5 block text-xs font-medium text-foreground"${_scopeId}>Açıklama</label>`);
            _push2(ssrRenderComponent(RichTextEditor, {
              modelValue: unref(form).description,
              "onUpdate:modelValue": ($event) => unref(form).description = $event,
              placeholder: "Hizmet hakkında detaylı açıklama yazın...",
              height: "300px",
              error: errors.value.description || unref(form).errors.description
            ...
            _push2(`</div><div class="grid grid-cols-1 gap-4 sm:grid-cols-2"${_scopeId}><div${_scopeId}><label class="mb-1.5 block text-xs font-medium text-foreground"${_scopeId}>Fiyat</label><div class="relative"${_scopeId}><input${ssrRenderAttr("value", unref(form).price)} type="number" step="0.01" placeholder="0.00" class="${ssrRenderClass([{ "border-destructive focus-visible:ring-destructive": errors.value.price || unref(form).errors.price }, "flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 pr-10 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"])}"${_scopeId}><span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground"${_scopeId}>₺</span></div>`);
            ...
            _push2(`</div></div></div></div><div class="flex items-center justify-between gap-2"${_scopeId}><button type="button" class="inline-flex h-9 items-center justify-center gap-1.5 rounded-md border border-destructive bg-background px-4 text-xs font-medium text-destructive ring-offset-background transition-colors hover:bg-destructive hover:text-destructive-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"${_scopeId}><svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"${_scopeId}></path></svg> Sil </button><div class="flex gap-2"${_scopeId}>`);
            ...
            _push2(`<button type="submit" class="inline-flex h-9 items-center justify-center gap-1.5 rounded-md bg-primary px-5 text-xs font-medium text-primary-foreground ring-offset-background transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""}${_scopeId}>`);
            if (unref(form).processing) {
              _push2(`<svg class="h-3.5 w-3.5 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"${_scopeId}><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"${_scopeId}></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"${_scopeId}></path></svg>`);
            } else {
              _push2(`<svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"${_scopeId}></path></svg>`);
            }
            ...
            return [
              createVNode("div", { class: "mx-auto max-w-4xl" }, [
                createVNode("div", { class: "mb-6" }, [
                  createVNode("h1", { class: "text-xl font-semibold text-foreground" }, "Hizmeti Düzenle"),
                  createVNode("p", { class: "mt-1 text-xs text-muted-foreground" }, toDisplayString(service.value.name), 1)
                ]),
                createVNode("form", {
                  ...
                  createVNode("div", { class: "rounded-lg border border-border bg-card p-5 shadow-sm" }, [
                    createVNode("div", { class: "space-y-4" }, [
                      createVNode("div", null, [
                        createVNode("label", { class: "mb-1.5 block text-xs font-medium text-foreground" }, "Hizmet Adı"),
                      ...
                      createVNode("div", null, [
                        createVNode("label", { class: "mb-1.5 block text-xs font-medium text-foreground" }, "Açıklama"),
                      ...
                      createVNode("div", { class: "grid grid-cols-1 gap-4 sm:grid-cols-2" }, [
                        createVNode("div", null, [
                          createVNode("label", { class: "mb-1.5 block text-xs font-medium text-foreground" }, "Fiyat"),
                          createVNode("div", { class: "relative" }, [
                            withDirectives(createVNode("input", {
                              ...
                              step: "0.01",
                              placeholder: "0.00",
                              class: ["flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 pr-10 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50", { "border-destructive focus-visible:ring-destructive": errors.value.price || unref(form).errors.price }]
                            }, null, 10, ["onUpdate:modelValue"]), [
                              [vModelText, unref(form).price]
                            ]),
                            createVNode("span", { class: "absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground" }, "₺")
                          ]),
                        ...
                        createVNode("div", null, [
                          createVNode("label", { class: "mb-1.5 block text-xs font-medium text-foreground" }, "Üst Kategori"),
