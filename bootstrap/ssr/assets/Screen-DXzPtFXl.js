      ...
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<div class="p-4"${_scopeId}><div class="mb-4 flex justify-between"${_scopeId}><div${_scopeId}><h2 class="text-xl font-bold text-base-content"${_scopeId}>${ssrInterpolate(__props.languagePack.name)}</h2><p class="text-sm text-base-content/70"${_scopeId}>${ssrInterpolate(__props.languagePack.description)}</p><p class="mt-1 text-sm text-base-content/70"${_scopeId}><span class="font-medium"${_scopeId}>Dil:</span> ${ssrInterpolate(getLanguageName(__props.languagePack.language))}</p></div><div class="flex space-x-2"${_scopeId}>`);
            if (isLoggedIn.value) {
              _push2(`<button class="btn btn-primary"${_scopeId}> Kelime Ekle </button>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<a${ssrRenderAttr("href", _ctx.route("rendition.language-packs.export", __props.languagePack.id))} class="btn btn-success"${_scopeId}> Dışa Aktar </a><a${ssrRenderAttr("href", _ctx.route("rendition.language-packs.index"))} class="btn btn-outline"${_scopeId}> Geri Dön </a></div></div><div class="overflow-x-auto"${_scopeId}><table class="table table-zebra w-full"${_scopeId}><thead${_scopeId}><tr${_scopeId}><th class="border-b px-4 py-3 text-left text-sm font-semibold"${_scopeId}>Kelime</th><th class="border-b px-4 py-3 text-left text-sm font-semibold"${_scopeId}>Anlam</th><th class="border-b px-4 py-3 text-left text-sm font-semibold"${_scopeId}>Tür</th><th class="border-b px-4 py-3 text-left text-sm font-semibold"${_scopeId}>Zorluk</th><th class="border-b px-4 py-3 text-left text-sm font-semibold"${_scopeId}>İşlemler</th></tr></thead><tbody${_scopeId}>`);
            if (!__props.languagePack.words || __props.languagePack.words.length === 0) {
              _push2(`<tr${_scopeId}><td colspan="5" class="text-center py-6 text-base-content/70"${_scopeId}> Bu dil paketinde henüz kelime bulunmamaktadır. Kelime ekleyin. </td></tr>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<!--[-->`);
            ssrRenderList(__props.languagePack.words, (word) => {
              ...
              if (isLoggedIn.value) {
                _push2(`<button class="btn btn-error btn-xs"${_scopeId}> Kaldır </button>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</td></tr>`);
            ...
            if (showAddWordsModal.value) {
              _push2(`<div class="modal modal-open"${_scopeId}><div class="modal-box max-w-2xl"${_scopeId}><div class="mb-4 flex items-center justify-between"${_scopeId}><h3 class="text-lg font-bold"${_scopeId}>Kelime Ekle</h3><button class="btn btn-ghost btn-sm"${_scopeId}><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"${_scopeId}></path></svg></button></div><div class="mb-4"${_scopeId}><label class="label"${_scopeId}><span class="label-text"${_scopeId}>Kelime Ara</span></label><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Kelime veya anlam ara..." class="input input-bordered w-full"${_scopeId}></div><div class="mb-4"${_scopeId}><label class="label"${_scopeId}><span class="label-text"${_scopeId}>Kelimeler</span></label>`);
              if (loading.value) {
                ...
                _push2(`<div class="py-4 text-center text-base-content/70"${_scopeId}> Eklenebilecek kelime bulunamadı. Önce yeni kelimeler ekleyin. </div>`);
              } else {
                _push2(`<div class="max-h-60 overflow-y-auto border border-base-300 rounded-lg p-2"${_scopeId}><!--[-->`);
                ssrRenderList(filteredAvailableWords.value, (word) => {
                  _push2(`<div class="flex items-center rounded px-2 py-1 hover:bg-base-200"${_scopeId}><input${ssrRenderAttr("id", `word-${word.id}`)}${ssrIncludeBooleanAttr(Array.isArray(selectedWords.value) ? ssrLooseContain(selectedWords.value, word.id) : selectedWords.value) ? " checked" : ""}${ssrRenderAttr("value", word.id)} type="checkbox" class="checkbox checkbox-primary"${_scopeId}><label${ssrRenderAttr("for", `word-${word.id}`)} class="ml-2 block text-sm text-base-content"${_scopeId}>${ssrInterpolate(word.word)} - ${ssrInterpolate(word.meaning)} (${ssrInterpolate(getLanguageName(word.language))}) </label></div>`);
                });
                _push2(`<!--]--></div>`);
              }
              _push2(`</div><div class="modal-action"${_scopeId}><button class="btn btn-outline"${_scopeId}> İptal </button><button class="btn btn-primary"${ssrIncludeBooleanAttr(selectedWords.value.length === 0 || processing.value) ? " disabled" : ""}${_scopeId}>${ssrInterpolate(processing.value ? "Ekleniyor..." : "Ekle")}</button></div></div></div>`);
            } else {
              _push2(`<!---->`);
            }
          } else {
              ...
              createVNode("div", { class: "p-4" }, [
                createVNode("div", { class: "mb-4 flex justify-between" }, [
                  createVNode("div", null, [
                    ...
                    createVNode("p", { class: "text-sm text-base-content/70" }, toDisplayString(__props.languagePack.description), 1),
                    createVNode("p", { class: "mt-1 text-sm text-base-content/70" }, [
                ...
                createVNode("div", { class: "modal-box max-w-2xl" }, [
                  ...
                  createVNode("div", { class: "mb-4" }, [
                    createVNode("label", { class: "label" }, [
                      createVNode("span", { class: "label-text" }, "Kelime Ara")
                    ]),
                  ...
                  createVNode("div", { class: "mb-4" }, [
                    createVNode("label", { class: "label" }, [
                      createVNode("span", { class: "label-text" }, "Kelimeler")
                    ]),
                    loading.value ? (openBlock(), createBlock("div", {
                      ...
                      key: 2,
                      class: "max-h-60 overflow-y-auto border border-base-300 rounded-lg p-2"
                    }, [
