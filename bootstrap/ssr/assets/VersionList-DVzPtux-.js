      ...
      ssrRenderList(__props.versions, (version) => {
        _push(ssrRenderComponent(unref(Link), {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<h3 class="${ssrRenderClass([isActive(version) ? "text-primary-foreground" : "text-foreground", "text-sm font-medium leading-tight"])}"${_scopeId}>${ssrInterpolate(version.version)}</h3><div class="${ssrRenderClass([isActive(version) ? "text-primary-foreground/70" : "text-muted-foreground", "flex items-center gap-3 text-xs"])}"${_scopeId}><span class="flex items-center gap-1"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"${_scopeId}></path></svg><span class="truncate"${_scopeId}>${ssrInterpolate(formatDate(version.release_date))}</span></span></div>`);
            } else {
              return [
                createVNode("h3", {
                  class: ["text-sm font-medium leading-tight", isActive(version) ? "text-primary-foreground" : "text-foreground"]
                }, toDisplayString(version.version), 3),
                createVNode("div", {
                  class: ["flex items-center gap-3 text-xs", isActive(version) ? "text-primary-foreground/70" : "text-muted-foreground"]
                }, [
