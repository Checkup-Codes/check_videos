...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      var _a, _b;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "group rounded-lg border border-border bg-card transition-all hover:border-primary/40 hover:shadow-sm" }, _attrs))}><div class="flex items-center justify-between gap-3 p-4"><div class="${ssrRenderClass([{ "cursor-pointer": hasChildren.value }, "flex flex-1 items-center gap-3 min-w-0"])}"><div class="h-10 w-10 shrink-0 overflow-hidden rounded-md border border-border bg-muted/30">`);
      if ((_b = (_a = __props.service.images) == null ? void 0 : _a[0]) == null ? void 0 : _b.image_path) {
        _push(ssrRenderComponent(_sfc_main$1, {
          src: __props.service.images[0].image_path,
          alt: __props.service.name,
          gallery: __props.service.images,
          "wrapper-class": "h-full w-full",
          "img-class": "h-full w-full object-cover"
        }, null, _parent));
      } else {
        _push(`<div class="flex h-full w-full items-center justify-center"><svg class="h-4 w-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg></div>`);
      }
      _push(`</div><div class="min-w-0 flex-1"><div class="flex items-center gap-2"><h3 class="text-sm font-semibold text-foreground truncate group-hover:text-primary transition-colors">${ssrInterpolate(__props.service.name)}</h3>`);
      ...
      if (__props.service.description) {
        _push(`<p class="mt-1 line-clamp-1 text-xs text-muted-foreground">${ssrInterpolate(unref(stripHtml)(__props.service.description))}</p>`);
      } else {
        _push(`<!---->`);
      }
