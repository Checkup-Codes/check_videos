...
Lütfen resmi küçültüp tekrar deneyin.`);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "w-full" }, _attrs))} data-v-a34e6248>`);
      if (__props.label) {
        _push(`<label class="mb-1 block text-sm font-medium text-foreground" data-v-a34e6248>${ssrInterpolate(__props.label)}</label>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="quill-editor-container rounded-md border border-input bg-background" style="${ssrRenderStyle({ height: __props.height })}" data-v-a34e6248></div>`);
      if (__props.error) {
        _push(`<p class="mt-1 text-xs text-destructive" data-v-a34e6248>${ssrInterpolate(__props.error)}</p>`);
      } else {
        _push(`<!---->`);
      }
      if (pendingImages.value.length > 0) {
        _push(`<div class="mt-2 rounded-md border border-amber-500/20 bg-amber-500/5 p-2" data-v-a34e6248><div class="flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400" data-v-a34e6248><svg class="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-a34e6248><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" data-v-a34e6248></path></svg><span data-v-a34e6248>${ssrInterpolate(pendingImages.value.length)} resim kaydetme bekliyor (Formu kaydettiğinizde yüklenecek)</span></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<p class="mt-1 text-xs text-muted-foreground" data-v-a34e6248> Maksimum dosya boyutu: 2MB. Desteklenen formatlar: JPEG, PNG, GIF, WebP, SVG </p></div>`);
    };
  }
};
...
const RichTextEditor = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-a34e6248"]]);
export {
