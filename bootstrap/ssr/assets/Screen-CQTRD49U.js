...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const keywords = ref(
      "yazılım öğrenme yolları, yazılım öğrenme siteleri, yazılım öğrenme platformları, yazılım öğrenme kitapları, yazılım öğrenme videoları, yazılım öğrenme uygulamaları"
    );
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "grid grid-cols-1 p-5 leading-loose" }, _attrs))}><div><div class="rounded-lg bg-white p-3 shadow-lg">`);
      if (loading.value) {
      ...
      _push(`<button${ssrIncludeBooleanAttr(currentPage.value === totalPages.value) ? " disabled" : ""} class="${ssrRenderClass([currentPage.value === totalPages.value ? "bg-gray-800 text-white" : "bg-gray-200 text-gray-800", "rounded px-3 py-2 text-sm transition-all hover:bg-gray-300"])}">${ssrInterpolate(totalPages.value)}</button></div></div><div class="mt-6 rounded-lg bg-white p-6 shadow-lg"><h2 class="mb-4 text-2xl font-semibold text-gray-800">Keywords</h2><div><textarea class="h-40 w-full rounded border bg-gray-50 p-4 outline-none focus:ring-2 focus:ring-gray-500" placeholder="Keywordleri virgülle ayrılmış şekilde girin" rows="4">${ssrInterpolate(keywords.value)}</textarea><div class="flex justify-end"><button class="mt-3 w-1/3 rounded bg-gray-800 py-2 text-white transition-all hover:bg-gray-700"> Kopyala </button></div></div></div></div></div>`);
    };
  }
};
