  ...
  setup(__props) {
    ...
    const handleSubmit = () => {
      ...
      form.post("/projects", {
        onSuccess: (page) => {
          var _a;
          const projectId = ((_a = page.props.project) == null ? void 0 : _a.id) || page.url.split("/").pop();
          if (projectId) {
      ...
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(` ${ssrInterpolate(unref(form).processing ? "Kaydediliyor..." : "Projeyi Kaydet")}</button></div></form></div></div>`);
          } else {
            return [
                                  ...
                                  (openBlock(true), createBlock(Fragment, null, renderList(getServiceTodos(service.id), (todo, index) => {
                                    return openBlock(), createBlock("div", {
                                      ...
                                      withDirectives(createVNode("input", {
                                        ...
                                        onBlur: ($event) => updateTodoTitle(service.id)
