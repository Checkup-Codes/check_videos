      ...
      _push(ssrRenderComponent(_sfc_main$1, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<!--]--></tbody></table></div><div class="mt-4 flex flex-col items-center gap-2 sm:flex-row sm:justify-between"${_scopeId}><div${_scopeId}><span class="text-sm opacity-70"${_scopeId}>${ssrInterpolate(paginationInfo.value)}</span></div><div class="join"${_scopeId}><button class="join-item btn btn-sm"${ssrIncludeBooleanAttr(currentPage.value <= 1) ? " disabled" : ""}${_scopeId}> « </button><button class="join-item btn btn-sm"${_scopeId}>${ssrInterpolate(currentPage.value)}</button><button class="join-item btn btn-sm"${ssrIncludeBooleanAttr(currentPage.value >= totalPages.value) ? " disabled" : ""}${_scopeId}> » </button></div></div></div></div>`);
          } else {
            return [
