    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (__props.show) {
        ...
        ssrRenderList(__props.duplicates, (duplicate, index) => {
          ...
          _push(`</div><div class="ml-4"><input type="checkbox"${ssrRenderAttr("id", `word-${index}`)}${ssrIncludeBooleanAttr(Array.isArray(selectedWords.value) ? ssrLooseContain(selectedWords.value, duplicate.id) : selectedWords.value) ? " checked" : ""}${ssrRenderAttr("value", duplicate.id)} class="h-4 w-4 rounded border-2 border-input bg-background text-primary ring-offset-background focus:ring-2 focus:ring-ring focus:ring-offset-2 dark:border-muted-foreground"></div></div>`);
        });
        _push(`<!--]--></div><div class="mb-6 flex items-center gap-2"><input type="checkbox" id="select-all"${ssrIncludeBooleanAttr(selectedWords.value.length === __props.duplicates.length) ? " checked" : ""} class="h-4 w-4 rounded border-2 border-input bg-background text-primary ring-offset-background focus:ring-2 focus:ring-ring focus:ring-offset-2 dark:border-muted-foreground"><label for="select-all" class="text-sm font-medium text-foreground"> Tümünü Seç (${ssrInterpolate(selectedWords.value.length)}/${ssrInterpolate(__props.duplicates.length)}) </label></div><div class="flex items-center justify-end gap-3"><button class="rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-accent"> İptal </button><button${ssrIncludeBooleanAttr(selectedWords.value.length === 0 || processing.value) ? " disabled" : ""} class="rounded-lg bg-primary px-6 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">${ssrInterpolate(processing.value ? "Ekleniyor..." : `${selectedWords.value.length} Kelimeyi Paketlere Ekle`)}</button></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
    };
