    ...
    codeBlocks.forEach((container, blockIndex) => {
      ...
      if (codeLines.length > 0) {
        ...
        Array.from(codeLines).forEach((line, index) => {
          ...
          content = content.replace(/(\/\/.*$|\/\*[\s\S]*?\*\/)/gm, '<span class="comment">$1</span>').replace(/\b(function|return|if|else|for|while|var|let|const|class|import|export|default|from|async|await)\b/g, '<span class="keyword">$1</span>').replace(/(['"`])(.*?)\1/g, '<span class="string">$1$2$1</span>').replace(/\b(\d+(\.\d+)?)\b/g, '<span class="number">$1</span>').replace(/\b([a-zA-Z_]\w*)\s*\(/g, '<span class="function">$1</span>(').replace(/([+\-*/%=<>!&|])/g, '<span class="operator">$1</span>');
