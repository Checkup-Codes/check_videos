  ...
  <CheckSubsidebar :isNarrow="isNarrow">
    ...
    <div class="relative z-10 shrink-0 border-b border-border bg-background p-3">
      <div class="flex items-center justify-between gap-2">
        <span class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Yer İmleri</span>
      </div>
    </div>
    <!-- Scrollable Content -->
    <SubSidebarScreen ref="scrollableRef" class="sidebar-content-embedded min-h-0 flex-1" :infoClass="'flex-1 min-h-0'">
      <CategoryList :categories="categories" :currentUrl="url" />
    </SubSidebarScreen>
  </CheckSubsidebar>
</template>
...
import CategoryList from '@/Pages/Bookmarks/_components/CategoryList.vue';
