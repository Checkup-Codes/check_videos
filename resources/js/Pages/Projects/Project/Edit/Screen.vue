<template>
  <ProjectsPageFrame title="Projeyi Düzenle" :description="project.project_name">
                    ...
                    <div class="space-y-2">
                      ...
                      >
                        ...
                        <input
                          ...
                          @blur="updateTodo(service.id, index)"
                        />
                        <button
                          type="button"
...
// Initialize form and service data from project
  ...
  if (project.value.services && project.value.services.length > 0) {
    project.value.services.forEach((service) => {
      ...
      servicesData.value[service.id] = {
        ...
        guest_description: service.pivot?.guest_description || '',
        service_start_date: service.pivot?.service_start_date || null,
        service_end_date: service.pivot?.service_end_date || null,
      };
...
const removeTodo = async (serviceId, index) => {
  ...
  if (todo.id) {
    // Delete existing todo in background
    try {
      ...
      // Revert on error - add back to array
...
const toggleTodo = async (serviceId, index) => {
  ...
  if (todo.id) {
    // Update existing todo in background
    try {
...
const handleSubmit = () => {
  ...
  form.put(`/projects/${project.value.id}`, {
    onSuccess: () => {
      // Save any new todos that weren't saved yet
