...
class CommodityPriceController extends Controller
{
    ...
    public function latest(Request $request): JsonResponse
    {
        ...
        // Always hit the database for latest values (no caching)
        $results = $this->fetchLatestPrices($symbols, $request);

        $supportedCommodities = array_values(array_unique(array_map(
     ...
     */
    private function fetchLatestPrices(array $symbols, Request $request): array
    {
        $results = [];
        $unit = 'gram';
        $now = Carbon::now();
        ...
        foreach ($symbols as $symbolConfig) {
            ...
            $query = CommodityPrice::where('provider_symbol', $providerSymbol);
